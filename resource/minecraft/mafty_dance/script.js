...
function prepareScene() {
	...
	light = new THREE.DirectionalLight( 0xffe2b9, 0.6 );
	...
	LOADER_1.load (
		...
		function ( mmd ) {
			...
			for ( const KEY in mesh.morphTargetDictionary ) {
				...
				htmlSource += '>'
				htmlSource += mesh.morphTargetDictionary[ KEY ]
...
window.addEventListener( 'resize', function () {
	...
	canvasSizeH = Number( window.getComputedStyle( document.getElementById( 'webgl' ) ).width.replace( /[^0-9]/g, '' ) );
